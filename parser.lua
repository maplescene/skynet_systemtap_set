...
local function location(state)
   return setmetatable({
      ...
      local s = string.format("@line:%d column:%d offset:%d", t.line, t.column, t.offset)
      return s
   end})
end
...
local closing_tokens = {
   ["end"] = true, ["eof"] = true, ["else"] = true, ["elseif"] = true, ["until"] = true,
}
...
function parse_block(state, loc)
   local block = {tag = "Block", location = loc}
